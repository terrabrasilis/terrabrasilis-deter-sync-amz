  ...
  DB="deter_cerrado"
	...
	then
  	...
  	QUERY_PUBLIC="SELECT $OUTPUT_COLUMNS FROM (SELECT gid, classname, quadrant, orbitpoint as path_row, image_date as view_date, sensor, satellite, areatotalkm, areamunkm, areauckm, county as municipality, uf, uc, geom FROM terrabrasilis.deter_processed_fm limit 1 ) as tb1 WHERE tb1.areatotalkm >= "
		QUERY_AUTH="SELECT $OUTPUT_COLUMNS, tb1.areatotalkm FROM (SELECT gid, classname, quadrant, orbitpoint as path_row, image_date as view_date, sensor, satellite, areatotalkm, areamunkm, areauckm, county as municipality, uf, uc, geom FROM terrabrasilis.deter_processed_fm ) as tb1 WHERE tb1.areatotalkm >= "
	else
