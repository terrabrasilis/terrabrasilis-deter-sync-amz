...
then
	...
	QUERY_PUBLIC="SELECT $OUTPUT_COLUMNS FROM (SELECT origin_gid||'_0' as gid, classname, quadrant, path_row, view_date, created_date, sensor, satellite, areatotalkm, areauckm, uc, areamunkm, county as municipality,  uf, geom FROM deter_cerrado_mun_ucs WHERE view_date <= (SELECT date FROM public.deter_publish_date) UNION SELECT origin_gid||'_1' as gid, classname, quadrant, path_row, view_date, created_date, sensor, satellite, areatotalkm, areauckm, uc, areamunkm, county as municipality,  uf, geom FROM deter_cerrado_history) as tb1 WHERE tb1.areatotalkm >= "
	QUERY_AUTH="SELECT $OUTPUT_COLUMNS FROM (SELECT origin_gid||'_0' as gid, classname, quadrant, path_row, view_date, created_date, sensor, satellite, areatotalkm, areauckm, uc, areamunkm, county as municipality,  uf, geom FROM deter_cerrado_mun_ucs UNION SELECT origin_gid||'_1' as gid, classname, quadrant, path_row, view_date, created_date, sensor, satellite, areatotalkm, areauckm, uc, areamunkm, county as municipality,  uf, geom FROM deter_cerrado_history) as tb1 WHERE tb1.areatotalkm >= "
else
	DB="DETER-B"
