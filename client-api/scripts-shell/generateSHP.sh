  ...
  DB="deter_cerrado"
	...
	then
  	...
  	QUERY_PUBLIC="SELECT $OUTPUT_COLUMNS FROM (SELECT gid, classname, quadrant, orbitpoint as path_row, image_date as view_date, sensor, satellite, areatotalkm, areamunkm, areauckm, county as municipality, uf, uc, geom FROM terrabrasilis.deter_processed_fm limit 1 ) as tb1 WHERE tb1.areatotalkm >= "
		...
		PUBLIC_FILTER="$ALL_FILTER AND date_audit <= (SELECT date FROM public.deter_publish_date)"
  	QUERY_PUBLIC="SELECT $OUTPUT_COLUMNS FROM (SELECT gid||'_curr' as gid, classname, quadrant, orbitpoint as path_row, date as view_date, lot, sensor, satellite, areatotalkm, areamunkm, areauckm, county as municipality, uf, uc, geom FROM deter_table WHERE $PUBLIC_FILTER UNION SELECT gid||'_hist', classname, quadrant, orbitpoint as path_row, date as view_date, lot, sensor, satellite, areatotalkm, areamunkm, areauckm, county as municipality, uf, uc, geom FROM deter_history) as tb1 WHERE tb1.areatotalkm >= "
		QUERY_AUTH="SELECT $OUTPUT_COLUMNS, tb1.areatotalkm as areatotkm FROM (SELECT gid||'_curr' as gid, classname, quadrant, orbitpoint as path_row, date as view_date, lot, sensor, satellite, areatotalkm, areamunkm, areauckm, county as municipality,  uf, uc, geom FROM deter_table WHERE $ALL_FILTER UNION SELECT gid||'_hist', classname, quadrant, orbitpoint as path_row, date as view_date, lot, sensor, satellite, areatotalkm, areamunkm, areauckm, county as municipality, uf, uc, geom FROM deter_history) as tb1 WHERE tb1.areatotalkm >= "
	fi;
fi;
